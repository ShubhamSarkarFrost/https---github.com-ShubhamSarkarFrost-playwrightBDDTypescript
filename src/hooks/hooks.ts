...
BeforeAll(async function () {
  browser = await invokeBrowser(process.env.BROWSER_TYPE);
});
...
After(async function ({ pickle, result }) {
  ...
  img = await fixture.page.screenshot({
    path: `./test-results/screenshots/${pickle.name}/${formattedDate}-${pickle.name}.png`,
    type: 'png',
  });
  ...
  await context.close();
  if (result?.status == Status.PASSED || Status.FAILED) {
